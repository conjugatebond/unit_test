...
public:
	...
	// stride = 1;
		...
		//auto end = Clock::now();
		//uint64_t dt = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
		//printf("MSM Cuda kenel took %f ms\n", dt/1000.0);
		//printf("end_launch_coop: %ld \n", end);
                if (i < batch - 1) {
                    h_off += stride;
