...
extern "C" {
    ...
    //cout<<"snarkvm_msm_check_here"<<endl;
    //printf("snarkvm_msm_check_here");
	...
	//cout<<"snarkvm_g_check_here"<<endl;
        if (!snarkvm_g.ok()) {
            return RustError{cudaErrorMemoryAllocation};
        }
    ...
    //std::cout <<"time different: ®" << std::chrono::duration_cast<std::chrono::milliseconds>(tp2-tp1).count() << std::endl;
}
#endif // __CUDA_ARCH__
