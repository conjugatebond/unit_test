    ...
    pub fn prove(
        ...
        for nonce in nonces {
            ...
            let h = thread::spawn(move || {
                let polynomial = Self::prover_polynomial(&epoch_challenge, address, n);
                tx.send((polynomial.unwrap(), n));
            });
            th.push(h);
        }
        ...
        println!("prover_polynomial..., time consumption: {:?}", duration);

        for _h in th {
            //let polynomial:DensePolynomial<<N::PairingCurve as PairingEngine>::Fr> = rx.recv().unwrap();
            let (polynomial, nonce) = rx.recv().unwrap();

            //println!("polynomial len: {:#?} ", polynomial);
            //println!("challenge degree: {:#?} ",  epoch_challenge.degree());
            ...
            //println!("evaluation len:   {:#?}---", duration);
            ...
            //let t2 = SystemTime::now();
            //let duration = t2.duration_since(t1);
            //println!("commit_lagrange: checkpoint..., time  consumption: {:?}", duration);
            ...
            return Ok(ProverSolution::new(partial_solution, proof));
        }
        return Err(anyhow!("can't found solution"));
    }
